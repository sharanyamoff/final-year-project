    ...
    while True:
        ...
        try:
            ...
            prediction_class = str(
                prediction.get("prediction", "BENIGN")
            ).upper()
            ...
            dqn = prediction.get("dqn") or {}

            action = str(
                dqn.get("action", "ALLOW")
            ).upper()

            with telemetry_lock:

                if prediction_class == "BENIGN":
                    telemetry["normal_count"] += 1
                else:
                    telemetry["attacks_count"] += 1
                ...
                telemetry["packets_per_sec"] = float(
                    job["features"].get(
                        "flow_packets_per_s",
                        0.0
                    )
